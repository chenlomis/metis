...
Scoring system prompt order: identity → candidate brief → full rendered profile → feedback → bullet rules → rubric. The identity anchors the model's perspective before it reads data. For feedback parsing (`_llm_process`, with `_claude_process` kept as a compatibility alias), grounding rules go in the `system=` parameter and the analysis format goes in the user turn — this prevents the analysis task from overriding the identity constraints.
...
**D-64 · LLM provider boundary normalizes provider names and keeps model choices per stage**
Public AI tasks now go through `metis.llm`: `complete_text()` accepts a provider-neutral client, `create_llm_client()` builds Anthropic/OpenAI adapters, and `resolve_stage_models()` chooses model names per provider and stage. Anthropic remains the default and best-tested provider. OpenAI is supported across `metis`, `metis init`, `metis feedback`, and tracker LLM fallback, but still needs score-parity and output-quality calibration before it should be considered equivalent.

`METIS_LLM_PROVIDER` is deliberately provider-only. Values are normalized for common casing/separator variants (`openAI`, `open_ai`, `Anthropic`, `Claude`), while model-family names such as Haiku/Sonnet/GPT belong in `ANTHROPIC_MODEL`, `ANTHROPIC_PRESCREEN_MODEL`, `OPENAI_MODEL`, etc. Provider-specific variables win over generic `MODEL`/`PRESCREEN_MODEL`/`EXTRACT_MODEL`; generic built-in defaults from another provider are ignored during provider switches so `METIS_LLM_PROVIDER=openai` does not accidentally send Claude model IDs to OpenAI.
