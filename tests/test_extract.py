"""Tests for metis/extract.py — Layer 1 JD extraction.
...
class TestBlankStruct:
    ...
    def test_blank_struct_has_required_keys(self):
        from metis.extract import _BLANK_STRUCT, _REQUIRED_KEYS
...
class TestIsValidStruct:
    def test_valid_struct_passes(self):
        from metis.extract import _is_valid_struct
...
class TestFormatExtractionForScoring:
    ...
    def test_blank_struct_returns_empty(self):
        from metis.extract import format_extraction_for_scoring, _BLANK_STRUCT
...
class TestExtractJdStructs:
    ...
    def test_large_batch_chunks_correctly(self):
        ...
        from metis.extract import extract_jd_structs, _EXTRACT_CHUNK_SIZE
    ...
    def test_short_response_padded_with_blank_structs(self):
        ...
        from metis.extract import extract_jd_structs, _BLANK_STRUCT
        client = MagicMock()
        ...
        assert result[1]["jd_quality"] == "extraction_failed"    # padded — JD may exist; must not trigger jd_blank gate
        assert result[2]["jd_quality"] == "extraction_failed"

    def test_api_failure_returns_extraction_failed_structs(self):
        """If API raises, extraction falls back with extraction_failed — not blank — so jd_blank gate does not fire."""
        import anthropic
        from metis.extract import extract_jd_structs
        client = MagicMock()
        ...
        with patch("time.sleep"):
            result = extract_jd_structs(client, [_make_job(), _make_job()])
        assert len(result) == 2
        assert all(r["jd_quality"] == "extraction_failed" for r in result)

    def test_broken_json_returns_extraction_failed_structs(self):
        from metis.extract import extract_jd_structs
        r = MagicMock()
...
class TestPipelineExtractionIntegration:
    ...
    def test_gate_filtered_job_gets_filtered_verdict(self):
        ...
        from metis.pipeline import _stage_enrich_and_score
        from metis.extract import _BLANK_STRUCT

        with patch("metis.sources.linkedin.enrich_jobs", return_value=[_make_job()]), \
             patch("metis.extract.extract_jd_structs", return_value=[dict(_BLANK_STRUCT)]), \
             patch("metis.profile.load_profile_yaml", return_value={}), \
             patch("metis.pipeline.score_jobs_batch") as mock_score, \
             patch("metis.pipeline.rank_jobs", side_effect=lambda jobs: jobs):
    ...
    def test_salary_gated_job_skips_sonnet(self):
        ...
        with patch("metis.sources.linkedin.enrich_jobs", return_value=[_make_job()]), \
             patch("metis.extract.extract_jd_structs", return_value=[below_floor]), \
             patch("metis.profile.load_profile_yaml", return_value={"salary_floor_usd": 150000}), \
             patch("metis.pipeline.score_jobs_batch") as mock_score, \
             patch("metis.pipeline.rank_jobs", side_effect=lambda jobs: jobs):
            result = _stage_enrich_and_score([_make_job()], MagicMock())
    ...
    def test_passing_job_reaches_sonnet(self):
        ...
        with patch("metis.sources.linkedin.enrich_jobs", return_value=[_make_job()]), \
             patch("metis.extract.extract_jd_structs", return_value=[_minimal_struct()]), \
             patch("metis.profile.load_profile_yaml", return_value={}), \
             patch("metis.pipeline.score_jobs_batch", side_effect=fake_score), \
             patch("metis.pipeline.rank_jobs", side_effect=lambda jobs: jobs):
            result = _stage_enrich_and_score([_make_job()], MagicMock())
    ...
    def test_extraction_failure_falls_back_gracefully(self):
        ...
        with patch("metis.sources.linkedin.enrich_jobs", return_value=[_make_job()]), \
             patch("metis.extract.extract_jd_structs", side_effect=RuntimeError("API down")), \
             patch("metis.profile.load_profile_yaml", return_value={}), \
             patch("metis.pipeline.score_jobs_batch", side_effect=fake_score), \
             patch("metis.pipeline.rank_jobs", side_effect=lambda jobs: jobs):
            result = _stage_enrich_and_score([_make_job()], MagicMock())
    ...
    def test_mixed_gate_and_passing_jobs_separated_correctly(self):
        ...
        with patch("metis.sources.linkedin.enrich_jobs", return_value=jobs), \
             patch("metis.extract.extract_jd_structs", return_value=structs), \
             patch("metis.profile.load_profile_yaml", return_value={}), \
             patch("metis.pipeline.score_jobs_batch", side_effect=fake_score), \
             patch("metis.pipeline.rank_jobs", side_effect=lambda jobs: jobs):
...
# ---------------------------------------------------------------------------
# init_bak_cmd.py — _apply_prefs_to_profile location_preference
# ---------------------------------------------------------------------------
...
class TestFeedbackCmd:
    ...
    def test_save_and_load_feedback_roundtrip(self, tmp_path, monkeypatch):
        monkeypatch.setattr("metis.feedback.FEEDBACK_FILE", tmp_path / "feedback.md")
        monkeypatch.setattr("metis.feedback.DATA_DIR", tmp_path)
        from metis.feedback import save_feedback_entry, load_feedback_text
    ...
    def test_save_last_run_roundtrip(self, tmp_path, monkeypatch):
        monkeypatch.setattr("metis.feedback.LAST_RUN_FILE", tmp_path / "last_run.json")
        monkeypatch.setattr("metis.feedback.DATA_DIR", tmp_path)
    ...
    def test_load_last_run_returns_none_when_missing(self, tmp_path, monkeypatch):
        monkeypatch.setattr("metis.feedback.LAST_RUN_FILE", tmp_path / "no_file.json")
